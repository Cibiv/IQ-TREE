...
//    ASSERT(std::isfinite(*df) && "Numerical underflow for lh-derivative");
    ...
    if (!asymptotics_computed) {
        ...
        double sum_derivative = 0;

        int size_block = int(nstates * ncat * vector_size);
        ...
        for (i = 0; i < number_blocks; ++i) {
            for (j = 0; j < int(vector_size) and vector_size * i + j < aln->size(); ++j) {
                if (theta_all[size_block*i + vector_size*position_zero + j] <= 0)
                    cout <<  "Warning: A coefficient associated to a zero eigenvalue is zero." << endl;
                for (k = 0; k < amount_of_equal_eigenvalues; ++k) {
                    sum_derivative +=  ptn_freq[vector_size * i + j] * theta_all[size_block*i + vector_size*positions_of_biggest[k]+j]/theta_all[size_block*i + vector_size*position_zero+j];
                }
            }
        }
        asymptotic_derivative = sum_derivative;
        //cout << endl << "This is the asymptotic of the derivative " << asymptotic_derivative << endl;
        ...
        for (i = 0; i < number_blocks; ++i) {
            for (j = 0; j < int(vector_size) and vector_size * i + j < aln->getNPattern(); ++j) {
                sum_likelihood +=  ptn_freq[vector_size * i + j] * log(theta_all[size_block*i+ vector_size*position_zero + j]);
            }
        }
        asymptotic_likelihood = sum_likelihood;
        //cout << endl << "This is the asymptotic of the likelihood " << asymptotic_likelihood << endl;
        
    
        double sum_upper_bound = 0;
        for (i = 0; i < aln->getNPattern(); ++i) {
            sum_upper_bound += ptn_freq[i]*(log(ptn_freq[i]) - log(aln->getNSite()));
        }
        upper_bound_likelihood = sum_upper_bound;
        
        //cout << endl << "This is the upper bound" << upper_bound_likelihood << endl;
        

        asymptotics_computed = true;
    }
}
